...
class Trainer:
    ...
    def __init__(self,
        ...
        if not os.exit("./checkpoints"):
            os.mkdir("./checkpoints")
        elif not os.exit("./checkpoints/weights") or not os.exit("./checkpoints/test_images"):
            os.mkdir("./checkpoints/weights")
            os.mkdir("./checkpoints/test_images")
    ...
    def test_step(self,
        ...
        for n in range(5):
            ...
            ax[n].axis("off")
        plt.tight_layout()
        plt.savefig(f"./checkpoints/test_images/{epoch}.png")
    ...
    def fit(self,
        ...
        with mlflow.start_run(run_name=self.mlf["run_name"]) as _:
            ...
            for epoch in tqdm.tqdm(range(epochs)):
                ...
                if epoch == 1:
                    mlflow.log_artifact("./plot_extract/actors/generate_labels.py")
                    mlflow.log_artifact("./plot_extract/models_zoo/cnn_lstm.py")
                if (epoch + 1) % 20 == 0:
                    saved_under = "./checkpoints/weights/intermediate.pth"
        ...
        saved_under = f"./checkpoints/weights{self.mlf['run_name']}.pth"
        torch.save(self.model.state_dict(), saved_under)
        mlflow.log_artifact(saved_under)
